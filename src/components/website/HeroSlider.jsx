...
export default function HeroSlider({ data }) {
  ...
  const primaryColor = DEFAULT_PRIMARY || '#1E3A8A';
  const accentColor = DEFAULT_ACCENT || '#FACC15';

  const content = data;
  const slides = content?.blocks && content.blocks.length > 0
    ? content.blocks
  ...
  return (
    <section className="relative h-[90vh] min-h-[600px] overflow-hidden bg-slate-900 group">
      ...
      <AnimatePresence>
        ...
        >
          ...
          <div 
            ...
            style={{ 
              backgroundImage: `url(${slide?.image || 'https://images.unsplash.com/photo-1580582932707-520aed937b7b?w=1920'})`,
            }}
          />
          ...
          {/* Content */}
          <div className="relative h-full max-w-7xl mx-auto px-4 flex items-center z-10">
              ...
              >
                {slide?.button1Text && (
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-8 py-4 font-semibold rounded-full transition-all hover:shadow-xl hover:-translate-y-1 group/btn hover:bg-yellow-400"
                ...
                {slide?.button2Text && (
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-8 py-4 border-2 border-white text-white font-semibold rounded-full hover:bg-white transition-all hover:text-[#1E3A8A]"
                  >
                    {slide.button2Text}
                  </Link>
                )}
            ...
            {slides.map((_, idx) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  idx === currentIndex ? 'w-8 bg-yellow-300' : 'w-2 bg-white/50 hover:bg-white'
                }`}
              />
            ))}
          </div>
