...
export default function ContactPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pb-24">
      {/* 1. Header Block */}
      <section className="relative w-full bg-[#1E3A8A] overflow-hidden flex items-center justify-center min-h-[300px] mb-16 shadow-inner">
        {header?.backgroundImage ? (
           <FixedImage src={header.backgroundImage} ratio="16/9" className="opacity-30 mix-blend-overlay w-full" />
        ) : (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 max-w-3xl mx-auto z-10 text-white">
          <h1 className="text-4xl md:text-6xl font-bold font-['Poppins'] mb-6 tracking-tight drop-shadow">{header?.heading}</h1>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div className="bg-white rounded-3xl shadow-[0_10px_40px_rgba(15,23,42,0.06)] p-8 md:p-12 border border-slate-100">
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                {parsedFormFields?.length > 0 && parsedFormFields.map((field, idx) => (
                  <div key={idx}>
                    ...
                    {field.toLowerCase().includes('message') || field.toLowerCase().includes('query') ? (
                       <textarea rows="4" value={formValues[normalizeFieldKey(field)] || ''} onChange={(e) => handleFieldChange(field, e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent transition text-gray-900 bg-gray-50/50" placeholder={`Enter your ${field.toLowerCase()}...`}></textarea>
                    ) : (
                       <input type={field.toLowerCase().includes('email') ? 'email' : 'text'} value={formValues[normalizeFieldKey(field)] || ''} onChange={(e) => handleFieldChange(field, e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#1E3A8A] focus:border-transparent transition text-gray-900 bg-gray-50/50" placeholder={`Enter ${field.toLowerCase()}...`} />
                    )}
                  </div>
                ))}
                {submitState.success && (
                ...
                <button type="submit" disabled={submitState.loading} className="w-full py-4 mt-4 bg-[#1E3A8A] text-white font-bold rounded-xl hover:bg-blue-900 transition shadow-md transform hover:-translate-y-1 disabled:cursor-not-allowed disabled:opacity-70">
            ...
            <div className="flex flex-col justify-center space-y-10">
               <div>
                 <h2 className="text-4xl font-bold text-gray-900 mb-4 font-['Poppins']">{infoSide?.heading || "Reach Out"}</h2>
                 {infoSide?.subheading && <h3 className="text-xl font-medium text-[#1E3A8A] mb-4">{infoSide.subheading}</h3>}
                 {infoSide?.description && <p className="text-lg text-gray-600 leading-relaxed whitespace-pre-wrap">{infoSide.description}</p>}
               </div>

               {cards?.length > 0 && (
                 <div className="grid gap-6">
                   {cards.map((card, idx) => (
                     <div key={idx} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex items-start space-x-5 hover:shadow-md transition">
                       <div className="w-12 h-12 bg-white shadow-sm rounded-full flex flex-shrink-0 items-center justify-center text-[#1E3A8A]">
                       ...
                       <div>
                         ...
                         <p className="text-slate-600 mt-1 whitespace-pre-wrap">{card.text}</p>
                       </div>
                     </div>
                   ))}
                 </div>
